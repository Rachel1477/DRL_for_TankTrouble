...
namespace TankTrouble
{
    ...
    DodgeStrategy AgentSmith::tryRotatingWithMoving(uint64_t globalSteps)
    {
        ...
        DodgeStrategy::DodgeOperation bestMoveOp = (direction == MOVING_FORWARD) ? DodgeStrategy::DODGE_CMD_MOVE_FORWARD : DodgeStrategy::DODGE_CMD_MOVE_BACKWARD;
        uint64_t rs = 0;
        uint64_t ms = 0;
        DodgeStrategy::DodgeOperation rotateCwOp = (direction == MOVING_FORWARD) ? DodgeStrategy::DODGE_CMD_FORWARD_CW : DodgeStrategy::DODGE_CMD_BACKWARD_CW;
        DodgeStrategy::DodgeOperation rotateCcwOp = (direction == MOVING_FORWARD) ? DodgeStrategy::DODGE_CMD_FORWARD_CCW : DodgeStrategy::DODGE_CMD_BACKWARD_CCW;
                                          ...
                                          int witchSide, int angleGran)
    {
        ...
        if (v1.cross(vt) != 0)
            ...
            direction = util::angleBetweenVectors(v1, vt) <= 90.0 ? MOVING_FORWARD : MOVING_BACKWARD;
        DodgeStrategy::DodgeOperation moveOp = (direction == MOVING_FORWARD) ? DodgeStrategy::DODGE_CMD_MOVE_FORWARD : DodgeStrategy::DODGE_CMD_MOVE_BACKWARD;
        uint64_t s;
        ...
        for (int i = 0; i < 90 / Tank::ROTATING_STEP; i++)
        {
            ...
            {
                moveOp = pointingToSameSide ? DodgeStrategy::DODGE_CMD_MOVE_FORWARD : DodgeStrategy::DODGE_CMD_MOVE_BACKWARD;
                direction = pointingToSameSide ? MOVING_FORWARD : MOVING_BACKWARD;
            }
            else if (v1.cross(vn) * v1.cross(vt) < 0)
            {
                moveOp = pointingToSameSide ? DodgeStrategy::DODGE_CMD_MOVE_BACKWARD : DodgeStrategy::DODGE_CMD_MOVE_FORWARD;
            ...
            else
            {
                ...
                else
                {
                    direction = ((witchSide == RIGHT_SIDE && v1.cross(vn) > 0) ||
                                 (witchSide == LEFT_SIDE && v1.cross(vn) < 0))
                                    ? MOVING_FORWARD
                                    : MOVING_BACKWARD;
                    moveOp = (direction == MOVING_FORWARD) ? DodgeStrategy::DODGE_CMD_MOVE_FORWARD : DodgeStrategy::DODGE_CMD_MOVE_BACKWARD;
                }
            }
            if (tryMovingStraight(step, direction, tryPos, &s))
            {
    ...
    void AgentSmith::getDodgeStrategy(const Object::PosInfo &pos, uint64_t globalSteps)
    {
        ...
        ctl->updateStrategy(finalStrategy, this);
    }
                            ...
                            uint64_t globalSteps)
    {
        ...
        if (util::distanceOfTwoPoints(smith.pos, enemy.pos) <= ATTACKING_RANGE)
        {
            ...
            if (aimingPos.isValid())
            {
                ...
                ctl->updateStrategy(strategy, this);
