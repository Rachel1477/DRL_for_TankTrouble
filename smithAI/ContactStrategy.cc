...
{
    bool ContactStrategy::update(LocalController *ctl, Tank *tank, uint64_t globalStep,AgentSmith* agent)
    {
        ...
        if (tankPos == prevPos)
        {
            ...
            stuckSteps = 0;
        prevPos = tankPos;
        if (!agent->safeToMove(globalStep, tankPos, tank->getMovingStatus()))
            tank->stop();
        return true;
    }
}